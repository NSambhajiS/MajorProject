...
def maybe_start_outbound_call():
    auto_dial = os.getenv("AUTO_DIAL_ON_START", "false").lower() in {"1", "true", "yes"}
    if not auto_dial:
        return
    ...
    if missing:
        print(f"Auto dial skipped. Missing env vars: {', '.join(missing)}")
        return

    try:
        from twilio.rest import Client
    except ImportError:
        print("Auto dial skipped. Install Twilio SDK: pip install twilio")
        return
    ...
    print(f"Outbound call started: {call.sid}")
...
# voice agent
# sending to deepgram from twilio
...
# receiving from deepgram to twilio
...
async def main():
    ...
    await asyncio.to_thread(maybe_start_outbound_call)
